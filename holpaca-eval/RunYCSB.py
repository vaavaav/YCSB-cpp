    ...
    def __init__(
        ...
        self.used_mem = getMem(
            sum(
                [
                    int(
                        config.get(f"cachelib.size.{i}", config.get("cachelib.size", 0))
                        * config.get(
                            f"cachelib.pool.relsize.{i}",
                            config.get("cachelib.pool.relsize", 1),
                        )
                    )
...
def runLOCAL(name, setup: Setup, db_backup, outdir, status):
    if DRY_RUN:
        ...
        print(
            f"[DRY-RUN] [LOCAL] {setup.build_cmd(status)} > {os.path.join(outdir, 'ycsb.txt')}"
        )
        if setup.controller_exec:
    ...
    with open(os.path.join(outdir, "dstat.csv"), "w") as dstat_output:
        ...
        if setup.controller_exec:
            ...
            controller = subprocess.Popen(
                [
                    setup.controller_exec,
                    setup.controller_ip,
                    *setup.controller_args.split(),
                ],
...
def runSIF(
    name, setup: Setup, db_backup, outdir, status, load_job_id, sif_path=None, binds=[]
):
    if not os.path.exists(sif_path):
        raise FileNotFoundError(f"SIF file not found: {sif_path}")
...
def runSIFController(
    name, setup, db_backup, outdir, status, load_job_id, sif_path=None, binds=[]
):
    if not os.path.exists(sif_path):
        raise FileNotFoundError(f"SIF file not found: {sif_path}")
...
def runSIFControllerMultiple(
    ...
    for setup in setups:
        ...
        local_ycsb_output = "/tmp/ycsb.txt"
        # Prepare the output paths for dstat
        dstat_output = os.path.join(setup.out, "dstat.csv")
        # Prepare the output paths for YCSB
        ycsb_output = os.path.join(setup.out, "ycsb.txt")
        # List commands to copy workload files to the local storage
        copy_workloads_cmd = []
        for thread, tracefile in enumerate(setup.traces):
            if tracefile:
                # Redirect the trace file to a local path
                setup.config[f"trace.file.{thread}"] = (
                    f"/tmp/{os.path.basename(tracefile)}"
                )
                # Add command to copy the trace file to the local storage
                copy_workloads_cmd.append(f"cp '{tracefile}' /tmp")
        # Get the command to copy the database files to the local storage
