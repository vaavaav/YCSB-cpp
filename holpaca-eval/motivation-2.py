...
if __name__ == '__main__':
    ...
    outputDir = os.path.join(os.path.abspath(sys.argv[3]), f"motivation-2", f"{datetime.now().strftime('%m-%d-%H-%M-%S')}")
    ...
    outputDir = os.path.join(os.path.abspath(sys.argv[3]), f"motivation-2-1k", f"{datetime.now().strftime('%m-%d-%H-%M-%S')}")
    RunYCSB(sourceDir, workloads, outputDir, load, setups, runs, status, sifDir)
