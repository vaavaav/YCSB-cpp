...
const std::string PROP_CACHE_TYPE = "cachelib.type";
const std::string PROP_CACHE_TYPE_DEFAULT = "baseline";
...
void CacheLibHolpacaOverhead::Init() {
    ...
    std::visit(
        [&](auto &config) {
          ...
          if (props_->GetProperty(
            ...
            if (!address.empty()) {
              holpacaConfig.setAddress(address);
            }
            ...
            if (!controllerAddress.empty()) {
              holpacaConfig.setControllerAddress(controllerAddress);
            }
          }
  ...
  std::visit(
      [&poolName, &poolSize](auto &&cache) {
        CacheLibHolpacaOverhead::poolId_ = cache->addPool(
            poolName,
