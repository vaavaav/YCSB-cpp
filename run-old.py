...
#    'sleepafterload' : 0,
    ...
    'cachelib.cachesize': 2_000_000, # in bytes
    'status.interval': 1,
    'readallfields': 'false',
    'fieldcount' : 1,
    'fieldlength': 1000,
    ...
    'holpaca.host.0': '192.168.112.124:6000',
    'holpaca.host.1': '192.168.112.124:6001',
    'holpaca.host.2': '192.168.112.124:6002',
    'holpaca.host.3': '192.168.112.124:6003',
}
...
for setup in setups:
    setups[setup]['overrideConfigs'] = {**setups[setup]['overrideConfigs'], **keyRangePerThread(ycsb, threads)}
...
def loadDB(workload, settings):
    command = f'{executable_dir}/ycsb -load -db cachelib-holpaca -P {workloads_dir}/{workload} -threads {settings["threadcount"]} {" ".join([f"-p {k}={v}" for k,v in settings.items()])}'
...
def runBenchmark(workload, settings, output_file, outputDir):
    ...
    command = f'systemd-run --scope -p MemoryMax={ycsb_settings["cachelib.cachesize"]*1.1} --user {executable_dir}/ycsb -run -db cachelib-holpaca -P {workloads_dir}/{workload} -s {status} -threads {threads} {" ".join([f"-p {k}={v}" for k,v in ycsb_settings.items()])}'
    print('\tCleaning heap')
