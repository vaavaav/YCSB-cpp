...
HdrHistogramMeasurements::HdrHistogramMeasurements() {
  for (int op = 0; op < MAXOPTYPE; op++) {
    if (hdr_init(0, 100LL * 1000 * 1000 * 1000, 3, &histogram_[op]) != 0) {
...
std::string HdrHistogramMeasurements::GetCDF() {
  ...
  msg_stream << "Mean latency (ms):"
             << hdr_mean(histogram_[Operation::ALL]) / 1000.0 << std::endl;
  msg_stream << "[ ALL:";
  for (int j = 1; j <= 99; j++) {
    msg_stream << " P" << j << "="
               << hdr_value_at_percentile(histogram_[Operation::ALL],
