...
public:
  ...
  CoreWorkload()
      : field_count_(0), read_all_fields_(false), write_all_fields_(false),
        ...
        field_chooser_(nullptr), scan_len_chooser_(nullptr),
...
protected:
  ...
  virtual DB::Status TransactionUpdate(DB &db, std::string const &key,
                                       size_t objectSize);
  virtual DB::Status TransactionInsert(DB &db, std::string const &key,
                                       size_t objectSize);
